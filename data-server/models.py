...
class PointsHistory(db.Model):
    ...
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True, comment='主键ID')
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, comment='用户ID')
    points_change = db.Column(db.Integer, nullable=False, comment='积分变化（正数增加，负数减少）')
    reason = db.Column(db.String(100), comment='变化原因')
    related_type = db.Column(db.String(50), comment='关联类型（skincare_record/report等）')
    related_id = db.Column(db.String(100), comment='关联ID')
    balance_after = db.Column(db.Integer, comment='变化后余额')
    created_at = db.Column(db.DateTime, default=datetime.now, comment='创建时间')
