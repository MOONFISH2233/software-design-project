...
def check_redis_connection():
    """检查 Redis 连接"""
    print(Colors.OKBLUE + "[1/5] 检查 Redis 连接..." + Colors.ENDC)
...
def start_module(module_name, process_list):
    """启动单个模块"""
    try:
...
def main():
    ...
    # 启动各个模块
    print(Colors.OKBLUE + "\n[2/5] 启动数据接收模块..." + Colors.ENDC)
    start_module('module_receiver.py', process_list)
    time.sleep(0.5)
    
    print(Colors.OKBLUE + "[3/5] 启动数据验证模块..." + Colors.ENDC)
    start_module('module_validator.py', process_list)
    time.sleep(0.5)
    
    print(Colors.OKBLUE + "[4/5] 启动数据写入模块..." + Colors.ENDC)
    start_module('module_writer.py', process_list)
    time.sleep(0.5)
    
    print(Colors.OKBLUE + "[5/5] 启动模拟器客户端..." + Colors.ENDC)
    start_module('simulator_mq.py', process_list)
    time.sleep(1)
