...
function mergeRealtimeData(devices, skinPayload, environmentPayload) {
  const skinRows = sortByTimeDesc(getList(skinPayload, ['data', 'skinData', 'skin_data', 'records']));
  const environmentRows = sortByTimeDesc(getList(environmentPayload, ['data', 'environmentData', 'environment_data', 'records']));
  ...
  return devices.map((device) => {
    ...
    const ownLatest = device.latest || {};
    const latestSkin = latestSkinByDevice[device.device_id] || (device.data_type === 'skin' ? ownLatest : {});
    const latestEnvironment = latestEnvironmentByDevice[device.device_id] || (device.data_type === 'environment' ? ownLatest : {});
    ...
    if (isEnvironment) {
      return {
        ...
        latest_sensor_time: latestEnvironment.sensor_time || latestEnvironment.received_at || ''
      };
    }
    ...
    return {
      ...
      latest_sensor_time: latestSkin.sensor_time || latestSkin.received_at || ''
    };
  });
}
