...
Page({
  ...
  async fetchDeviceList(showLoading = true) {
    ...
    try {
      const deviceResult = await safeRequest('deviceRes', {
          url: '/device/list',
          method: 'GET',
          data: { page: 1, per_page: 20 }
      });
      const skinResult = await safeRequest('skinRes', {
          url: '/data/skin',
          method: 'GET',
          data: { days: 7, page: 1, per_page: 100 }
      });
      const environmentResult = await safeRequest('environmentRes', {
